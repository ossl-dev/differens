...
Structure buckets say shapes match. Content hashes say code matches. Within a structure bucket, nodes are compared by content hash: kind, label, value, and children. The hash is a candidate, not a verdict: an exact match is verified against the full subtree, so a forged collision cannot report a move that did not happen. A function moved unchanged to another file matches on both hashes. The correlator emits a `Move` with source and destination file paths instead of a `Delete` plus an `Insert`.

Jaccard similarity catches renames at a distance (stage 3).
...
Whole-file nodes score on content alone, not on the path in their label: a renamed file's two paths differ by definition, and scoring path tokens against each other diluted a one-token edit below the threshold, reporting a rename plus edit as a delete and an add.
...
The narration engine renders that as *"moved `computeTotal` from `src/utils.ts` to `src/helpers.ts`"*: one sentence instead of two unrelated hunks. An exported function moves as an unlabeled `Export` wrapper around the labeled `Function`; the correlator reports the move by the function's name, one level down.
