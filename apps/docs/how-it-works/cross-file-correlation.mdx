...
Structure buckets say shapes match. Content hashes say code matches. Within a structure bucket, nodes are compared by content hash: kind, label, value, and children. A function moved unchanged to another file matches on both hashes. The correlator emits a `Move` with source and destination file paths instead of a `Delete` plus an `Insert`.
