...
The router classifies each input file by extension and dispatches it to a tier adapter. `.ts` goes to T5 code, `.json` to T4 data, `.html` to T3 markup, `.txt` and `.log` to T2 prose, markdown (`.md`, `.mdx`) and everything else to T1 raw. Binary detection runs first on anything that fails the text sniff. The router is the only place that maps extensions to parsers. Adding a new format means adding a classification rule, not touching the pipeline.
...
Every tier can fall back to the tier below it. Unparseable code falls back to a generic tree-sitter CST diff, which falls back to an LCS line diff. Each fallback returns a diff instead of failing.
...
- **JSON**: full fidelity, with hash values serialized as integers.
...
The tradeoff is that every format pays the tree-matching cost, which is linear in node count. There are no input-size caps: a 100k-line file diffs one line at a time, a 3 MiB file is read in full, and trees of any node count are matched for real.
