...
The safety net: a line-level longest-common-subsequence diff over the raw text. It is the fallback for every tier above it. It is capped at 2,000 lines. Beyond that, T1 returns a whole-file `Update` action instead of running an unbounded LCS.
...
3. **T1 hits the 2,000-line cap** → returns a whole-file `Update` action.

The worst case for text is a line diff. It is less informative than a structural diff, but it still runs.
...
| T1 | Raw text, LCS line diff, 2,000-line cap | Whole-file `Update` |
