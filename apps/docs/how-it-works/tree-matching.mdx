...
The collision bound follows from the birthday problem: at one million nodes the chance of any two nodes colliding is about 0.006%, and at ten million about 0.6%. There is no node-count cap, so this bound applies at every realistic tree size.
