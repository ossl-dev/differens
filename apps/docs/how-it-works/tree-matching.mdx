...
Hashes use FNV-1a over the full 64-bit space. Each node's hash combines its own fields with its children's hashes, so a change anywhere in a subtree changes every ancestor's hash. That Merkle property makes subtree equality a single comparison.
...
The valves fail soft: exceeding a bound degrades the diff to fewer, larger matched nodes instead of aborting.
