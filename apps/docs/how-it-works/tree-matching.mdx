...
<Info>
Hashes are JavaScript numbers — not full 64-bit integers, so no `BigInt` handling is involved. The JSON formatter serializes them explicitly. See [Output](/guides/machine-outputs).
</Info>
