...
The TypeScript extractor covers both languages — one grammar for `.ts`/`.tsx`, another for `.js`/`.jsx`/`.mjs`/`.cjs` — and maps both onto the same canonical concepts.
...
<CodeGroup title="Refactor: rename, add parameter, move file">
<Code title="before.ts" language="typescript">
...
<Code title="after.ts" language="typescript">
export const TAX_RATE = 0.08;
</Code>
<Code title="helpers/math.ts" language="typescript">
