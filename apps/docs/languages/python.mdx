...
The Python extractor covers `.py` files. Python has one grammar construct for functions, so async functions parse as the same `function_definition` node — the `async` keyword is part of it — and both sync and async `def` become `Function`.
...
- **Assignments** — `assignment` and `augmented_assignment` become `Assignment` / `AugmentedAssignment`. Python has no separate `Variable` concept; a variable's name surfaces through its assignment's declarator, not a canonical label
...
<CodeGroup title="Refactor: extract, rename, add parameter">
<Code title="pricing.py (before)" language="python">
TAX_RATE = 0.08
...
<Code title="pricing.py (after)" language="python">
TAX_RATE = 0.08
...
<Code title="discounts.py (after)" language="python">
def apply_promo(price: float, max_discount: float = 0.5) -> float:
    return price * (1 - max_discount)
</Code>
<Code title="Output" language="text">
diff: pricing.py -> discounts.py
move: function `apply_discount` (across files)
rename: function `apply_discount` -> `apply_promo`
insert: parameter `max_discount` in function `apply_promo`
update: return expression in `apply_promo`
update: call expression in `checkout`: `apply_discount(total)` -> `apply_promo(total)`
</Code>
</CodeGroup>

The line diff shows `pricing.py` losing two functions and `discounts.py` gaining one. The semantic diff shows one function moved across files, renamed, and given a defaulted parameter — and `checkout` barely changed.

## Known limitations

- **Methods are not distinct** — there is no `Method` concept; a method is a `Function` nested in a `Class`.
- **No `Variable` concept** — plain assignments become `Assignment`, so renames of variables surface through their declarator children rather than a canonical label.
