...
Returns `{ changes: EditAction[], nodeCount: number, fallback?: "lines" }`. `fallback` appears only when the caller set `maxNodes` and a tree exceeds it; the default is `Infinity`, so trees of any size are matched for real. When a fallback is requested, the change list is empty. Line-diff instead in that case.
...
| `maxNodes` | `Infinity` | Opt-in node-count valve; when set, larger inputs fall back to a line diff |
