...
Differens is a semantic diffing engine. Instead of comparing lines of text, it parses your files into trees, matches nodes structurally, and tells you what actually changed — renamed, moved, extracted, reformatted — rather than which lines changed.
...
A rename, a move, an extraction, a reformat — every one of these shows up as a delete plus an insert. A one-word rename looks like the whole function was rewritten. A function moved to another file looks like it was deleted in one place and copy-pasted in another. Line diffs describe *where* text changed, never *what* changed.
...
4. **Narrate** — the narration engine turns the typed edit script into English: *"renamed `computeTotal` to `calculateTotalAmount` and added parameter `discount`"* — not *"removed 3 lines, added 5 lines"*.
