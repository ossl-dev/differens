...
Thanks for wanting to improve Differens. This guide covers cloning the repo, making changes, and shipping a release.
...
bun run test               # 292 unit, integration, and CLI e2e tests, must be zero failures
bun run test:coverage      # Coverage report (96%+ lines gated in CI)
bun run test:coverage-gate # Same gate CI runs: fails below 95% lines / 90% functions
```

Per-package tests run faster when you are iterating on one area, and watch mode
runs the suite again on every save:
...
The suites are layered: unit tests per module (`packages/*/src/*.test.ts`),
integration tests through the tier pipeline and the report layer, and e2e tests
that spawn the real CLI against throwaway git repositories
(`apps/cli/src/e2e.test.ts`). The git integration tests need the `git` binary
and create temp repos under the system temp directory; they never touch your
repositories.
...
Human-readable, past-tense, lowercase after the first word. Write it as a sentence describing what the diff did.
...
Adapters sit on the abstraction ladder: T0 is raw bytes, T5 is code. Each tier parses its content into a `Node` tree that the one diff core consumes. The core does not need to know where its input came from.
